...
export const getFilesystemCourseData = () => {
	if (isDev) {
		return buildCourse();
	}
...
export const listFilesystemChapters = (): ChapterSummary[] =>
	getFilesystemCourseData().chapters.map(({ slug, order, title, description, lessonCount }) => ({
		slug,
		order,
		title,
		description,
...
export const getFilesystemChapter = (chapterSlug: string): ChapterSummary | undefined => {
	const chapter = getFilesystemCourseData().chapters.find((item) => item.slug === chapterSlug);
...
export const listFilesystemLessonsForChapter = (chapterSlug: string): LessonSummary[] => {
	const chapter = getFilesystemCourseData().chapters.find((item) => item.slug === chapterSlug);
	return chapter ? chapter.lessons.map(toLessonSummary) : [];
};

export const getFilesystemLesson = (
	chapterSlug: string,
	lessonSlug: string
): PublicLesson | undefined => {
	const lesson = getFilesystemCourseData().lessonByChapterSlug.get(chapterSlug)?.get(lessonSlug);
	return lesson ? toPublicLesson(lesson) : undefined;
};

export const getFilesystemLessonDefinition = (lessonSlug: string): LessonDefinition | undefined =>
	getFilesystemCourseData().lessonBySlug.get(lessonSlug);

export const getFilesystemFirstLesson = (): CourseSequenceItem | undefined =>
	getFilesystemCourseData().sequence[0];

const getAdjacentFilesystemLesson = ({
	chapterSlug,
	lessonSlug,
	offset
}: {
	...
	const course = getFilesystemCourseData();
...
export const getFilesystemPreviousLesson = (chapterSlug: string, lessonSlug: string) =>
	getAdjacentFilesystemLesson({ chapterSlug, lessonSlug, offset: -1 });

export const getFilesystemNextLesson = (chapterSlug: string, lessonSlug: string) =>
	getAdjacentFilesystemLesson({ chapterSlug, lessonSlug, offset: 1 });
